...
async function runDemo() {
  ...
  try {
    // 1. Leer solicitud (¡CAMBIA ESTE NOMBRE por el real de tu carpeta en fixtures!)
    ...
    if (!solData.ok) {
      console.error("❌ Error leyendo solicitud:", solData.error);
      return;
    }

    console.log("✅ Formato solicitado:", solData.data?.solicitud?.formato);

    // 2. Mapear campos (si es xlsx)
    if(solData.data.plantilla) {
      const campos = solData.data.plantilla.map((p: any) => ({ etiqueta: p.etiqueta }));
      
      console.log(`Mapeando ${campos.length} campos...`);
      const mapRes = await mapear_campos.execute({ caso, campos_solicitados: campos }, ctx);
      const mapData = JSON.parse(mapRes);
      
      if (!mapData.ok) {
        console.error("❌ Error mapeando:", mapData.error);
        return;
      }

      console.log("✅ Campos llenos:", mapData.data?.llenos.length);
      console.log("⚠️ Campos faltantes:", mapData.data?.faltantes.length);
      console.log("❓ Requieren confirmación:", mapData.data?.requiere_confirmacion.length);
    }
  } catch (error) {
    console.error("💥 Error grave en demo.ts:", error);
  }
}
